...
class OCRProcessorSelect:
    def __init__(self):
        # 初始化YOLO模型
        self.yolo_model = self.load_yolo_model(r'C:\Users\Lenovo\pythonProject\aliOCR\select\runs\detect\train\weights\best.pt')
...
@app.route('/upload/no', methods=['POST'])
def upload_no():
    return process_image(request, processor_no)


@app.route('/upload/select', methods=['POST'])
def upload_select():
    return process_image(request, processor_select)


@app.route('/upload/program', methods=['POST'])
def upload_program():
    return process_image(request, processor_program)


def process_image(req, processor):
